...
# 下請け関数の定義
# iteration()関数
def iteration(u, point):
    """1回分の反復計算"""
    u_next = [[0 for i in range(n)] for j in range(m)]  # 次ステップのuij
    # 次のステップの値を計算
    for i in range(1, n-1):
        for j in range(1, m-1):
            u_next[i][j] = (u[i][j-1] + u[i-1][j] + u[i+1][j] + u[i][j+1] - point[i][j]) / 4
...
point = [[0 for i in range(n)] for j in range(m)]
point[int(n/4)][int(m/4)] = 100
point[int(n*3/4)][int(m*3/4)] = -100
...
# plt.contourf(X, Y, U, cmap=cm.coolwarm)            # contourマップ形式

savefile = "./png/gpoisson.png"
fig.savefig(savefile, format="png", dpi=300)

plt.show()
# gpoisson.pyプログラムの終わり