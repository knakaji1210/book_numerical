...
結果をグラフ描画する
初期状態ファイルには初期配置を記述する
使い方 % python3 glife.py < initlife.txt

ArtistAnimationを使ってgifアニメーションを作成する変更を加えた
ただし、計算時間はかなり遅くなる
...
imgs.append([w])    # gifアニメーションのために追加
...
for t in range(1, tmax):
    ...
    # グラフ出力
    w.set_data(world)           # 描画データの更新・・・このままだとうまくいかない
    w = plt.imshow(world, interpolation="nearest")    # gifアニメーションのために追加
    imgs.append([w])    # gifアニメーションのために追加
    plt.pause(0.01)
    

ani = animation.ArtistAnimation(fig, imgs, interval=100)          # gifアニメーションのために追加
ani.save('./gif/glife_ani.gif', writer = 'pillow', fps = 1000)    # gifアニメーションのために追加
